...
**robmqtt** — A resilient edge computing client with offline queuing over SQLite, exponential backoff, QoS 0/1/2 support, dual storage architecture, TLS and authentication, health check daemon and 24-hour soak tested. Built to survive real network failures, not just ideal conditions ( pip install robmqtt ).
